import React from 'react'
import { Image, Divider, Card, Row, Col } from 'antd';

const { Meta } = Card;

const pythonImgUrl = 'data:image/jpeg;base64,';

const Skills: React.FC = () => {
  return (  
    <div>
      <Row gutter={[16, 24]}>
      <Divider orientation='left'>Languages</Divider>
          <Col span={4}>
            <Card
              style={{ width: 140 }}
              cover={<img alt='react' src='https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/2300px-React-icon.svg.png' />}
            >
              <Meta title='React' />
            </Card>
          </Col>
          <Col span={4}>
            <Card
              style={{ width: 140 }}
              cover={<img alt='react' src='https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png' />}
            >
              <Meta title='JavaScript' />
            </Card>
          </Col>
          <Col span={4}>
            <Card
              style={{ width: 140 }}
              cover={<img alt='react' src='https://upload.wikimedia.org/wikipedia/commons/thumb/4/4c/Typescript_logo_2020.svg/2048px-Typescript_logo_2020.svg.png' />}
            >
              <Meta title='TypeScript' />
            </Card>
          </Col>
          <Col span={4}>
            <Card
              style={{ width: 140 }}
              cover={<img alt='react' src={pythonImgUrl} />}
            >
              <Meta title='Python' />
            </Card>
          </Col>
          <Col span={4}>
            <Card
              style={{ width: 140 }}
              cover={<img alt='react' src='https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/2300px-React-icon.svg.png' />}
            >
              <Meta title='React' />
            </Card>
          </Col>
          <Col span={4}>
            <Card
              style={{ width: 140 }}
              cover={<img alt='react' src='https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/2300px-React-icon.svg.png' />}
            >
              <Meta title='React' />
            </Card>
          </Col>
      <Divider orientation='left'>Frameworks & Libraries</Divider>
      </Row>
    </div>
  );
};

export default Skills;